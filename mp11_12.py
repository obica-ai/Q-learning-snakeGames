    ...
    def execute(self):
        if self.args.early_check:
            ...
            if not np.array_equal(test_results, gold_results):
                ...
                for (i, (t, g)) in enumerate(zip(test_results, gold_results)):
                    # if test_results[0] != gold_results[0]:
                    #     flag = True
                    if not np.array_equal(t, g):
                        print("Warning: Your discretized state",i,"does not match gold one")
                        print("Your discretized state:", t)
                        print("Gold discretized state:", g)
                        print("env:",envs[i])
                match = False
            else:
            ...
            for game in range(1, len(actions_list) + 1):
                ...
                if not np.array_equal(self.agent.Q, gold_Q):
                    ...
                    # print("Warning: Your Q table ", self.agent.Q)
                    # print("Warning: gold Q table ", gold_Q)
                    match = False
                if not np.array_equal(self.agent.N, gold_N):
                    print("Warning: Your N table does not match gold table for game", game)
                    # print("Warning: Your N table ", self.agent.N)
                    # print("Warning: gold N table ", gold_N)
                    match = False
